...
- GitHub Actions 每 5 分钟轻量保温、每 10 分钟安全检查，常见更新发现时间约 10 分钟；
...
公开仓库保存执行器令牌。工作流每 5 分钟访问一次不读取数据库的 `/livez`，
在 Render 的 15 分钟休眠阈值前保温；每 10 分钟才使用 OIDC 调用维护接口并
执行到期任务。这样 Neon 可在两次数据库检查之间按 5 分钟空闲规则缩容到零，
避免耗尽免费 CU-hours。免费服务没有
